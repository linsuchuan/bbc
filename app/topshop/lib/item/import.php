...
class topshop_item_import {
    ...
    protected function checkItems($items, $catId, $colsTitle)
    {
        ...
        foreach ($itemsTmp as $k=>$item)
        {
            ...
            $realItem['is_selfshop'] = $is_selfshop;
        ...
        {   
            ...
            if ($itv['spec_info'] && $specProps)
            {
                ...
                foreach ($skuInput as $k=>$text)
                {
                    ...
                    {
                        $spec_desc['spec_private_value_id'][$specTmps[$k][$text]['prop_id']] = '';
                ...
                $postdata[$sk]['sku_id'] = $sku['sku_id'] = 'new';
                $sku['spec_desc'] = $spec_desc;
                $postdata[$sk]['price'] = $sku['price'] = $itv['price'];
                $postdata[$sk]['mkt_price'] = $sku['mkt_price'] = $itv['mkt_price'];
                $postdata[$sk]['cost_price'] = $sku['cost_price'] = $itv['cost_price'];
                $postdata[$sk]['store'] = $sku['store'] = $itv['realStore'];
                $postdata[$sk]['bn'] = $sku['bn'] = '';
                if($itv['bn'])
                {
                    $postdata[$sk]['bn'] = $sku['bn'] = mb_substr($itv['bn'], 0, 30, 'utf-8');
                }
                
                $postdata[$sk]['barcode'] = $sku['barcode'] = '';
                if($itv['barcode'])
                {
                    $postdata[$sk]['barcode'] = $sku['barcode'] = mb_substr($itv['barcode'], 0, 30, 'utf-8');
                }
                $skus[$sk] = $sku;
